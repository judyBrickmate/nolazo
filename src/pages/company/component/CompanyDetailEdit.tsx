import { Button, Checkbox, FormControlLabel, FormGroup, StyledEngineProvider, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from '@mui/material';
...
export default function CompanyDetailEdit(props: any) {
      ...
      <StyledEngineProvider injectFirst>
        ...
        <Table sx={{ minWidth: 700 }} aria-label="customized table">
          ...
          <TableRow>
            ...
            <TableCell>
              <Link to={companyData.homePage}>{companyData.homePage}</Link>
            </TableCell>
          </TableRow>
        </Table>
          ...
          <TableBody>
            {companyData?.products.map((row: any, index: number) => (
              <TableRow key={row.id}>
                ...
                <TableCell align="center">{row.price.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ',')}</TableCell>
