            ...
            {listOrder.map((row: any) => (
              <TableRow key={row.id}>
                ...
                <TableCell scope="row" align="center">
                  <Link to={ROUTER.ORDER_DETAIL} state={{ id: row.id }} style={{ color: COLOR.BLACK }}>
                ...
                <TableCell align="center">{(row.iamport[0].refundAt !== null && `${moment(row.iamport[0].refundAt).format("YYYY-MM-DD  hh:mm:ss")}`) || "--"}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
